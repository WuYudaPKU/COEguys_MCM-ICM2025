...
本情境将仅作为对照组起到熟悉模型的作用。我们考虑没有化学品、播种与收割，但是有其他物种迁入的情形（因为其他物种迁入是生态系统成熟到一定程度的必然结果）。在这种情形下，我们得到了种群数量变化曲线如下图。

==1：没有化学品，没有其他物种迁入，以situation_1.csv命名==

[图：水稻、杂草、虫] [图：青蛙、鸟、蛇]

通过上图我们可以看到，在情境1下，生态系统正常发展，但是水稻不能长期存活，所以系统不能演变成一个农业生态系统。这说明了水稻与杂草的竞争能力以及对害虫的抵抗能力都比较弱，不能在没有人类干预的情况下稳定地存在于生态系统中。这个结果与现实相符，首次印证了模型的准确性。

#### 4.3 情境2：人类使用化学品或绿色农业方案

在情境2的讨论中，我们考虑一个真实的情境。现在有一个农民刚刚砍伐了一片森林用来种植水稻，起初，他使用除草剂和杀虫剂等化学用品抑制杂草和害虫，生态系统逐渐发展。当生态系统成熟到一定程度时，一些曾经从森林生态系统迁出的种群重新迁入，丰富了农田生态系统的食物网。生态系统再发展一段时间，已经变得比较稳定，农民开始不再使用除草剂和杀虫剂，转而使用绿色农业的方案，引进鸭子和蝙蝠到生态系统中。
...
我们将着重对比人类使用化学品（常规情境）和人类使用绿色农业方案（目标情境）时生态系统的演替情况。在两种情况下，生态系统在最开始的一段时间内都并不稳定，因此只展示生态系统呈现稳定模式后的动态情况。
