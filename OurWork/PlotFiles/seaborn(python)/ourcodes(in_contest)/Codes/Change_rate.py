...
# 读取 CSV 文件
file_path = r'D:\2025_PKUCOE_MCM\files_for_practice\seaborn(python)\ourcodes\sensibility\change_rate_ulti.csv'  # 更新文件路径
...
x_smooth, y_smooth = smooth_curve(time, data['0.35'])
sns.lineplot(x=x_smooth, y=y_smooth, label=r'$r=0.1$',linewidth=4,color=color_science_dict['crop'])

x_smooth, y_smooth = smooth_curve(time, data['0.3'])
sns.lineplot(x=x_smooth, y=y_smooth, label=r'$r=0.05$', linewidth=4,color=color_science_dict['weed'])

x_smooth, y_smooth = smooth_curve(time, data['0.25'])
sns.lineplot(x=x_smooth, y=y_smooth, label=r'$r=0$', linewidth=4,color=color_science_dict['straw'])
...
plt.title('Change of Natural Growth Rate of Rice Population', fontsize=30)
