...
# 读取 CSV 文件
file_path = r'files_for_practice/seaborn(python)/ourcodes/sensibility/change_noise.csv'  # 更新文件路径
...
x_smooth, y_smooth = smooth_curve(time, data['0.1'])
sns.lineplot(x=x_smooth, y=y_smooth, label=r'$\sigma=0.1$',color=color_science_dict['crop'],linewidth=4)

x_smooth, y_smooth = smooth_curve(time, data['0.05'])
sns.lineplot(x=x_smooth, y=y_smooth, label=r'$\sigma=0.05$', linewidth=4,color=color_science_dict['weed'])

x_smooth, y_smooth = smooth_curve(time, data['0'])
sns.lineplot(x=x_smooth, y=y_smooth, label=r'$\sigma=0$', linewidth=4,color=color_science_dict['straw'])
...
plt.title('Change of Noise Scale', fontsize=30)
